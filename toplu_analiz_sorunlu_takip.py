...
class FrekansBilgisi:
    ...
    def analiz_ekle(self, analiz_sonucu: dict):
        ...
        # 1. Eğer kaynağı 'varsayilan' ise sorunlu
        if analiz_sonucu.get('source') == 'varsayilan':
            self.sorunlu = True
            return
            
        # 2. Kaynağı 'zemberek' veya diğer güvenilir kaynaklar ise OK
        if analiz_sonucu.get('source') in ['zemberek', 'kokler_db', 'kendi_analiz']:
            self.sorunlu = False
            return
            
        # 3. Eğer kökü ve sözcüğün kendisi aynıysa ve boş ekler varsa, 
        # bu muhtemelen bir kök kelimedir, sorunlu değil
        if (analiz_sonucu.get('kok') == self.sozcuk and 
            (not analiz_sonucu.get('ekler') or len(analiz_sonucu.get('ekler', [])) == 0)):
            self.sorunlu = False
            return
            
        # 4. Diğer durumlarda, ekler boşsa sorunlu olabilir
        if not analiz_sonucu.get('ekler') or len(analiz_sonucu.get('ekler', [])) == 0:
            # Ancak bazı yaygın bağlaçlar, edatlar ve zarflar kök kelimedir, bunlar sorunlu değil
            cok_kullanilan_kok_sozcukler = {
                've', 'ile', 'bir', 'bu', 'şu', 'o', 'ben', 'sen', 'biz', 'siz', 
                'da', 'de', 'ki', 'mi', 'mı', 'mu', 'mü', 'ama', 'fakat', 'lakin',
                'çok', 'az', 'biraz', 'hiç', 'en', 'daha', 'en', 'pek', 'gayet',
                'için', 'gibi', 'kadar', 'diye', 'üzere', 'göre', 'rağmen', 'karşı',
                'önce', 'sonra', 'evvel', 'nihayet', 'artık', 'hemen', 'şimdi', 'henüz',
                'evet', 'hayır', 'yok', 'var', 'değil', 'tamam', 'peki', 'belki',
                'ne', 'kim', 'hangi', 'nasıl', 'nerede', 'nereye', 'nereden', 'niçin', 'neden',
                'veya', 'ya da', 'yahut', 'veyahut', 'hem', 'ya', 'ister', 'gerek'
            }
            
            if self.sozcuk.lower() in cok_kullanilan_kok_sozcukler:
                self.sorunlu = False
            else:
                self.sorunlu = True
...
def sorunlu_sozcukleri_kaydet(frekans_verileri: Dict[str, FrekansBilgisi], 
    ...
    if os.path.exists(veritabani_yolu):
        try:
            ...
            # Ana sorunlu_sozcukler tablosunu kontrol et
            try:
                cursor.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name='sorunlu_sozcukler'")
                tablo_sql = cursor.fetchone()
                
                # Eğer sozcuk sütunu UNIQUE kısıtlamasına sahip değilse, yeni tablo oluştur
                if tablo_sql and 'UNIQUE' not in tablo_sql[0]:
                    cursor.execute("DROP TABLE IF EXISTS sorunlu_sozcukler_yedek")
                    cursor.execute("CREATE TABLE sorunlu_sozcukler_yedek AS SELECT * FROM sorunlu_sozcukler")
                    cursor.execute("DROP TABLE sorunlu_sozcukler")
                    cursor.execute('''
                    CREATE TABLE sorunlu_sozcukler (
                        id INTEGER PRIMARY KEY,
                        sozcuk TEXT UNIQUE,
                        durum TEXT,
                        not_metni TEXT,
                    ...
                    ''')
                    cursor.execute("INSERT OR IGNORE INTO sorunlu_sozcukler SELECT * FROM sorunlu_sozcukler_yedek")
                    
                # Bu noktada 'sorunlu_sozcukler' tablosu UNIQUE kısıtlamasına sahip olacak
            ...
            for sozcuk, veri in sorunlu_sozcukler.items():
                try:
                    ...
                    (sozcuk, frekans, belge_frekansi, kaynak, belgeler, durum) 
                    VALUES (?, ?, ?, ?, ?, 'çözülemedi')
                    ''', (
                    ...
                    cursor.execute('''
                    INSERT OR REPLACE INTO sorunlu_sozcukler 
                    (sozcuk, durum, not_metni, deneme_sayisi) 
                    VALUES (?, 'çözülemedi', ?, 1)
                    ''', (
                        sozcuk, veri.get_belgeler_str()
                    ))
